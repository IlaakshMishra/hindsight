# Review package: Task 5 (no git — full file dump)
...
---
name: hindsight
description: Use when the user wants to save, search, list, or delete debugging lessons stored by the hindsight plugin, or types /hindsight. Subcommands - save, search <query>, list, prune <id>.
---
...
list everything saved, or prune a lesson that's stale or wrong.
...
`mcp__hindsight__list_lessons`, `mcp__hindsight__prune_lesson` — the
`mcp__<server>__<tool>` naming Claude Code gives every MCP tool). If a
tool call fails because the server isn't connected, tell the user the
`hindsight` MCP server isn't running rather than guessing at its output.
...
there's no subcommand, or it doesn't match one of the four below, show
...
just the delete side of that instead of the read/write side):
    ...
    delete_lesson(lesson_id, lessons_dir) -> bool
...
def delete_lesson(lesson_id: str, lessons_dir: Path | str) -> bool:
    ...
    raises on.
    """
    path = Path(lessons_dir) / f"{lesson_id}.md"
    if not path.exists():
        return False
    path.unlink()
    return True
...
## test additions (grep for new test names)
```
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:157:# --- delete_lesson ------------------------------------------------------------
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:160:def test_delete_lesson_removes_existing_file_and_returns_true(tmp_path: Path):
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:165:    result = store.delete_lesson("2026-01-01-to-delete", lessons_dir)
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:171:def test_delete_lesson_leaves_other_files_alone(tmp_path: Path):
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:176:    result = store.delete_lesson("2026-01-01-remove", lessons_dir)
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:183:def test_delete_lesson_returns_false_when_no_matching_file(tmp_path: Path):
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:187:    result = store.delete_lesson("2026-01-01-does-not-exist", lessons_dir)
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:193:def test_delete_lesson_returns_false_on_missing_lessons_dir(tmp_path: Path):
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_store.py:194:    result = store.delete_lesson("anything", tmp_path / "does-not-exist")
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:329:# --- prune_lesson (Task 5) ----------------------------------------------------
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:332:def test_prune_lesson_deletes_file_and_removes_it_from_search(isolated_project):
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:347:    result = main.prune_lesson(saved["id"])
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:356:def test_prune_lesson_returns_false_for_nonexistent_id_and_does_not_error(
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:359:    result = main.prune_lesson("2020-01-01-never-saved")
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:363:def test_prune_lesson_leaves_other_saved_lessons_searchable(isolated_project):
/Users/ilaakshmishra/Documents/hindsight/server/tests/test_main.py:372:    result = main.prune_lesson(remove["id"])
```

## server/tests/test_store.py (new delete_lesson tests only, full context)
```
...
## server/tests/test_main.py (full, incl new prune_lesson tests)
```
